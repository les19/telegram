...
abstract class BaseReception
{
    ...
    {
        if (is_null($command)) $command = $this->command;
        if (is_array($command) && !empty($command) && isset($command[0]) && isset($command[1])) {
            ...
            else $obj = app()->make($class, ['telegram' => $this->telegram, 'reception' => $this]);

            if (method_exists($obj, 'setTelegram') && method_exists($obj, 'getTelegram') && $obj->getTelegram() === null) $obj = $obj->setTelegram($this->telegram);
            if ($obj->getReception() === null) $obj = $obj->setReception($this);
    ...
    private function find()
    {
        ...
        } else if ($this->telegram->memory->hasNext()) {
