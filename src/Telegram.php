...
class Telegram
{
  ...
  {
    if (is_null($this->method)) $this->method = $method;
  ...
  public function keyboard(Keyboard|string|array $markup, array $keys = null): self
  {
    ...
    } else if (is_array($markup) && is_array($keys) && !empty($keys) && !empty($markup)) {
    ...
    } else if ($keyboard !== null && is_string($keyboard)) {
    ...
    foreach ($markup as $index => $row) {
      for ($i = 0; $i < $row; $i++) {
        if (count($keys) !== 0) {
          ...
          if($this->current_keyboard_type === 'inline_keyboard' && !isset($key['url']) && !isset($key['callback_data'])) $key['callback_data'] = $key['text'];
          $data[$index][] = $key;
        }
      }
    }
  ...
  public function hook(string|object $reception = null)
  {
    try {
      if (!$this->fromTelegram) {
        ...
        } else if (is_object($reception)) {
          $obj = $reception;
        }

        if (method_exists($obj, 'setTelegram') && method_exists($obj, 'getTelegram') && $obj->getTelegram() === null) $obj = $obj->setTelegram($this);
        $obj->call();
      }
    } catch (\Throwable $th) {
      report($th);
  ...
  public function setHook($url)
  {
    ...
    if (!str_contains($url, "https://") && str_contains($url, "http://")) $url = str_replace("http://", "https://", $url);
    $this->params = ['url' => $url];
    return $this;
  }
